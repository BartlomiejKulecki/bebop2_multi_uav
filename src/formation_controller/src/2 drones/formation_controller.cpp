...
void calculate_follower_trajectory(Drone& leader, Drone& f1)
{
	...
	printw("____ Trajektora followera zostala przeliczona ____ \n");
	printw("_________ GOTOWY DO STARTU TRAJEKTORII ___________ \n");
	refresh();
    ready = true;
}

// funkcja wykonujaca sie w watku obslugujacym klawiature 
void key_thread(Drone& L, Drone& f1) 
{
	ros::Rate r(30);
	int key;
	...
	while (ros::ok()) 
	{
		...
		else if ( ( key == '3') &&  L.flying && f1.flying &&  ready)
		{
...
// --------------------------- subscribers ---------------------------------	
    ...
    noecho();
	nodelay(stdscr, TRUE);
	thread Key_thread(key_thread, ref(leader), ref(follower1));  //start thread for keyboard service
	...
	switch(trajectory_mode){
		case 1:
			...
			calculate_follower_trajectory(leader, follower1);
			break;
		case 2:
			get_trajectory();
	...
	while (ros::ok())
	{
		...
		{	
			...
			if( leader.position_reached && follower1.position_reached  && !last_point) //	Jeśli doleciał do punktu to poślij mu następny
			{
			...
			else if( leader.position_reached && follower1.position_reached && last_point) // Jeśli doleciał do punktu i był to ostatni punkt to zdejmij flagę during_trajectory i wraca do wyboru opcji
			{
