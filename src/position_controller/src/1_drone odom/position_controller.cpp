...
void PositionController::update_pose(const nav_msgs::Odometry::ConstPtr& msg)
{
	position_controller::msgCoordinates conv;
	actual_pose.x = conv.x = -msg->pose.pose.position.y;
	actual_pose.y = conv.y = msg->pose.pose.position.x;
	actual_pose.z = conv.z = msg->pose.pose.position.z;
	q = msg->pose.pose.orientation;
	actual_pose.yaw = atan2(  (2*(q.w*q.z+q.x*q.y)) , (q.w*q.w - q.x*q.x - q.y*q.y - q.z*q.z) )  * 180 / PI;
	// obrót ukladu o 90 st (żeby układ odom pokrywał sie z lokalnym ukladem drona)
	if((actual_pose.yaw > 90.0)&&(actual_pose.yaw <=180))  
		actual_pose.yaw = conv.yaw = actual_pose.yaw -270.0;
	else
		actual_pose.yaw = conv.yaw = actual_pose.yaw + 90.0;
...
//------------------- subscribers -----------------------------------------------------
	pc.odom_sub = n.subscribe("/bebop_leader/odom", 100, &PositionController::update_pose, &pc);
