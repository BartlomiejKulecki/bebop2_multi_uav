...
class PositionController
{
	private:
		...
		double max_vel_xy, max_vel_z, max_vel_yaw;  // ograniczenia wyjścia regulatorow PID
...
void PositionController::update_pose(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
	...
	actual_pose.yaw = conv.yaw = -atan2(  (2*(q.w*q.y+q.x*q.z)) , (q.w*q.w - q.x*q.x - q.y*q.y - q.z*q.z) )  * 180.0/PI;
...
//------------------- subscribers -----------------------------------------------------
	pc.odom_sub = n.subscribe("/optitrack_node/optitrack0", 100, &PositionController::update_pose, &pc);
